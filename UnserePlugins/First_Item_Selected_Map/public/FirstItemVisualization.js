...
export default class FirstItemVisualization {
  constructor(el, vis) {
  ...
  renderMap(buckets) {
    ...
    loadModules([
      ...
      .then(([Map, SceneView, GraphicsLayer, Graphic]) => {
		...
		// for every session should be the first map center (when the user selected an item in the list) added to the map
        // depending on the appid
        for(var i=0; i<buckets.length; i++){

          var source = buckets[i].events.hits.hits[0]._source;
